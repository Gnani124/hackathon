...
import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Heart, Clock, Star, ChevronRight, Circle, Sparkles, Moon, Sun, Cloud, Wind, Waves, Leaf, Mountain, Sunrise, Sunset, Flower } from 'lucide-react-native';
...
const meditations = [
  {
    ...
    audio: require('@/assets/music/meditation-music-322801 (1).mp3'),
    category: 'Beginner',
...
const defaultMeditation: Meditation = {
  ...
  animation: 'https://example.com/animation.json',
  audio: require('@/assets/music/meditation-music-322801 (1).mp3'),
  category: 'General',
...
export default function Meditate() {
  ...
  const handlePlayPause = async () => {
    try {
      console.log('Play/Pause pressed, isAudioReady:', isAudioReady, 'sound:', !!sound);
      
      // If audio is not ready, try to initialize it
      if (!isAudioReady || !sound) {
  ...
  const handleMeditationChange = async (meditation: Meditation) => {
    try {
  ...
  const toggleMute = async () => {
    try {
      console.log('Toggling mute, isAudioReady:', isAudioReady, 'sound:', !!sound);
  ...
  // Add effect to handle meditation changes
  useEffect(() => {
    if (currentMeditation) {
      initializeAudio();
    }
  ...
  // Add effect to handle mute changes
  useEffect(() => {
    if (sound) {
      sound.setIsMutedAsync(isMuted).catch(error => {
        ...
        start={{ x: 0, y: 0 }}
          ...
          >
            <View style={styles.headerContent}>
              <Sparkles size={32} color="#ffffff" style={styles.headerIcon} />
              <View>
                <Text style={[
                  styles.title,
                    ...
                    <View style={styles.benefitsContainer}>
                      {currentMeditation?.benefits.map((benefit, index) => (
                        <View key={index} style={styles.benefitItem}>
                          {index === 0 ? <Heart size={16} color="#ffffff" style={styles.benefitIcon} /> :
                           index === 1 ? <Leaf size={16} color="#ffffff" style={styles.benefitIcon} /> :
                           <Sparkles size={16} color="#ffffff" style={styles.benefitIcon} />}
              ...
              <View style={styles.meditationInfo}>
                    <View style={styles.meditationHeader}>
                      <View style={styles.categoryBadgeSmall}>
                        {meditation.category === 'Beginner' ? <Sunrise size={14} color="#ffffff" style={styles.categoryIcon} /> :
                         meditation.category === 'Intermediate' ? <Sun size={14} color="#ffffff" style={styles.categoryIcon} /> :
                         meditation.category === 'Advanced' ? <Moon size={14} color="#ffffff" style={styles.categoryIcon} /> :
                         <Flower size={14} color="#ffffff" style={styles.categoryIcon} />}
