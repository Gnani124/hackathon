...
import { fetchExams, fetchExamResults } from '@/services/examsService';
...
export default function ExamsScreen() {
  ...
  const loadResults = async () => {
    try {
      const fetchedResults = await fetchExamResults(user?.id || '');
  ...
  const handleStudy = (exam: Exam) => {
    // Implement study materials functionality
    Alert.alert('Study Materials', 'Study materials will be available soon');
  };

  const renderResultsItem = ({ item }: { item: ExamResult }) => {
      ...
      <Card style={styles.resultCard}>
        <View style={styles.resultHeader}>
          <View style={styles.resultSubject}>
            <Text style={styles.resultSubjectText}>{exam?.subject || 'Unknown Subject'}</Text>
            <Text style={styles.resultTitle}>{exam?.title || 'Unknown Exam'}</Text>
          </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.tabs, { transform: [{ translateY: tabPosition }] }]}>
        ...
        >
          <Award size={20} color={activeTab === 'results' ? colors.primary : '#6B7280'} />
